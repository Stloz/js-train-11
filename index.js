...
function customSet(arr) {
  ...
  for (const el of set) {
    if (typeof el === "number") set.delete(el);
  }
  return set;
}
...
function setToArray(mySet) {
  ...
  const stringArray = arr.filter((el) => typeof el === "string");
...
function getDifference(set1, set2) {
  ...
  for (const el of set1) {
    if (!set2.has(el)) differenceSet.add(el);
  }
...
function getIntersection(arr1, arr2) {
  ...
  const set1 = new Set(arr1.values());
  const set2 = new Set(arr2.values());
  const intersectionSet = new Set();
  for (const el of set1) {
    if (set2.has(el)) intersectionSet.add(el);
  }
...
function iterateSet(set) {
  ...
  for (const entry of set.entries()) console.log(entry);
}
